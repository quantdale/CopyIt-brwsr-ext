# Repository-Local Add-ons Master Plan — CopyIt-brwsr-ext
...
Chrome/Edge MV3 extension plus Rust native-messaging host sharing CopyIt's SQLite/vault data, with Playwright and Windows certification paths.

**Decision:** `RECOMMEND_EXTENSION_DIAGNOSTICS_PLUS_DOCS`

Extension permissions and native-messaging origin checks are security authority; agent convenience must not weaken them.
...
**Why it fits:** Useful for extension popup/service-worker/console/network diagnostics and Chromium extension lifecycle debugging.

**Constraints:** Use a dedicated test browser profile with synthetic CopyIt data. Do not attach to the user's normal profile or expose real vault contents.

### 2. Context7 MCP — RECOMMEND

**Type:** MCP

**Why it fits:** Current TypeScript/Vite/Playwright and supporting library documentation.

**Constraints:** Docs only; no access to APPDATA or the real CopyIt database.

## Explicitly not recommended

- Filesystem MCP over %APPDATA%/CopyIt
- Browser MCP with normal-user profile access
- Replacing existing real Chrome/Edge certification scripts
